...
% Demon para ler temperatura (if_read)
def_readtd :-
...
classify(T, _Lai, Li, Ls, _Las, comfort) :- T >= Li, T < Ls.
classify(T, Lai, _Li, _Ls, _Las, freezing) :- T < Lai.
classify(T, _Lai, _Li, _Ls, Las, burning) :- T > Las.
classify(T, Lai, Li, _Ls, _Las, cold) :- T >= Lai, T < Li.
classify(T, _Lai, _Li, Ls, Las, hot) :- T >= Ls, T =< Las.

% Demon Control (if_write antes de atualizar)
def_controld :-
...
% Alarmes Críticos de Temperatura
...
def_sensors :-
    new_frame(sensor_temp), new_slot(sensor_temp, is_a, sensor),
    new_value(sensor_temp, type, temperature), new_value(sensor_temp, current_value, 20),

    new_frame(sensor_humidity), new_slot(sensor_humidity, is_a, sensor),
    new_value(sensor_humidity, type, humidity), new_value(sensor_humidity, current_value, 50),

    new_frame(sensor_co2), new_slot(sensor_co2, is_a, sensor),
    new_value(sensor_co2, type, co2), new_value(sensor_co2, current_value, 400),

    new_frame(sensor_presence), new_slot(sensor_presence, is_a, sensor),
    new_value(sensor_presence, type, presence), new_value(sensor_presence, current_value, 0),

    new_frame(sensor_door), new_slot(sensor_door, is_a, sensor),
    new_value(sensor_door, type, door_sensor), new_value(sensor_door, current_value, closed).

% -------------------------
% Sensors Demons
% Ativam todos depois de ser escrito um valor novo no current_value de um sensor, ativam um side_effect 
% -------------------------
def_sensor_demons :-
...
update_cooling(T, Ls) :- T > Ls, new_value(cooling_system, state, on).
update_cooling(T, Ls) :- T =< Ls, new_value(cooling_system, state, off).
...
humidity_control(_F, _S, Humidity, Humidity) :-
    ...
    new_value(humidifier, state, off),
    new_value(ventilation, state, off). % Nota: Sujeito a revisão concorrente com CO2
...
co2_control(_F, _S, CO2, CO2) :-
    CO2 =< 900,
    new_value(ventilation, state, off). % Nota: Sujeito a revisão concorrente com Humidade
...
adapt_limits(Z) :-
    ...
    NewLi is BaseLi + Z, NewLai is BaseLai + Z,
    NewLs is BaseLs + Z, NewLas is BaseLas + Z,
    new_value(thermo, li, NewLi), new_value(thermo, ls, NewLs),
    new_value(thermo, lai, NewLai), new_value(thermo, las, NewLas).
...
update_doors(open) :- new_value(automatic_doors, state, open).
update_doors(closed) :- new_value(automatic_doors, state, closed).

door_alarm(open) :- getdate(D), genmsg_general(sensor_door, door_open, open, info, D).
door_alarm(closed).
...
def_actuators :- 
    new_frame(cooling_system), new_slot(cooling_system, is_a, actuator),
    new_value(cooling_system, type, cooling), new_value(cooling_system, state, off),

    new_frame(ventilation), new_slot(ventilation, is_a, actuator),
    new_value(ventilation, type, ventilation), new_value(ventilation, state, off),

    new_frame(humidifier), new_slot(humidifier, is_a, actuator),
    new_value(humidifier, type, humidifier), new_value(humidifier, state, off),

    new_frame(alarm_system), new_slot(alarm_system, is_a, actuator),
    new_value(alarm_system, type, alarm), new_value(alarm_system, state, off),

    new_frame(automatic_doors), new_slot(automatic_doors, is_a, actuator),
    new_value(automatic_doors, type, doors), new_value(automatic_doors, state, closed).
